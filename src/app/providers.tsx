...
export function RecordsProvider({ children: rc }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      // Permanently remove children that have been in trash > 30 days (best-effort, owner only)
      softDeleted
  ...
  // Soft-delete: marks deletedAt, moves to trash. Permanent after 30 days.
  ...
  // Restore: removes deletedAt field, moves back to active list.
  const restoreChild = (id: string) => {
    const child = deletedList.find((c) => c.id === id);
    if (!child) return;
    const { deletedAt: _removed, ...restored } = child;
    setDeletedList((dl) => dl.filter((c) => c.id !== id));
