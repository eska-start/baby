...
사용자가 제공한 텍스트 또는 이미지(수첩, 병원 기록, 예방접종 증명서 등)에서 예방접종과 건강검진 정보를 추출해.
...
건강검진 차수: 1차(생후14~35일), 2차(4~6개월), 3차(9~12개월), 4차(18~24개월), 5차(30~36개월), 6차(42~48개월), 7차(54~60개월), 8차(66~71개월)
...
type OpenAIImageContent = { type: 'image_url'; image_url: { url: string; detail: 'auto' } };
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const body = (await req.json()) as { text?: string; birthDate?: string };
      text = body.text ?? '';
    ...
    if (imageBase64) {
      userContent.push({
        type: 'image_url',
        image_url: { url: `data:${imageMime};base64,${imageBase64}`, detail: 'auto' },
      });
    }

    const textPart = [
      birthDate ? `아이 생년월일: ${birthDate}` : '',
      text ? `--- 기록 내용 ---\n${text}` : '',
      imageBase64 ? '위 이미지에서 예방접종 및 건강검진 정보를 추출해주세요.' : '',
    ]
      .filter(Boolean)
      .join('\n\n');
