      ...
      {inviteUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setInviteUrl(null)}>
          <div className="w-full max-w-sm rounded-[20px] bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="break-all rounded-[12px] border border-line bg-bg px-3 py-3 font-mono text-[11px] text-ink select-all">
              {inviteUrl}
            </div>
            <button
              onClick={() => { navigator.clipboard.writeText(inviteUrl).then(() => { showToast("복사됐어요!"); setInviteUrl(null); }); }}
              className="mt-3 w-full rounded-[12px] bg-ink py-3 text-[13px] font-semibold text-white"
            >
              복사하기
            </button>
            ...
            {deletedChildren.length > 0 && (
              <div className="rounded-[18px] border border-line bg-card px-5 py-4 space-y-3">
                ...
                {deletedChildren.map((child) => {
                  const days = daysUntilPermanent(child.deletedAt!);
                  return (
                    <div key={child.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-ink-mute">{days}일 후 영구 삭제</div>
                      </div>
                      <button
                        onClick={() => { restoreChild(child.id); showToast(`${child.name} 복구됐어요`); }}
                        className="rounded-[10px] border border-line bg-bg px-3 py-1.5 text-[12px] font-semibold text-ink transition hover:bg-card"
                      >
                        복구
                      </button>
                    </div>
